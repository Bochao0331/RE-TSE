% demo_step4_cartesian_maxgirf_recon.m
% Written by Bochao Li
% Email: bochaoli@usc.edu
% Started: 08/15/2024, Last modified: 09/08/2024

%% Clean slate
clear all; clc;

%% Define a .json file saving dual data file(input file)
json_file = './raw/multi.json';
...
topup_correction_flag = json.recon_parameters.topup_correction_flag; % 1=yes, 0=no
...
% topup_sign
%--------------------------------------------------------------------------
if isfield(json, 'topup_sign_1')
    topup_sign(1) = json.topup_sign_1;
    topup_sign(2) = json.topup_sign_2;
else
    topup_sign = 1;
end
...
for idx = 1:nr_recons
    ...
    for nacq = 1:2 % run for each TSE acquisition
        ...
        %--------------------------------------------------------------
        cfl_file = fullfile(input_path{nacq}, sprintf('img_type1_slc%d_rep%d_%s_gridding%d_phc%d_conc%d_gnl%d_topup%d_i%d_l%4.2f', slice_number, repetition_number, slice_type, readout_gridding_flag, phase_correction_flag, conc_correction_flag, gnl_correction_flag, topup_correction_flag, maxiter, lambda));
        tstart = tic; fprintf('%s: Reading a .cfl file: %s... ', datetime, cfl_file);
        ...
        %----------------------------------------------------------------------
        % TOPUP displacement (Nkx x Nky x Nkz) saving the actual Siemens slice number
        %----------------------------------------------------------------------

        cfl_file = fullfile(topup_path, sprintf('displacement_slc%d_%s', slice_number, slice_type));
        tstart = tic; fprintf('%s: Reading a .cfl file: %s... ', datetime, cfl_file);
        displacement = readcfl(cfl_file);
        displacement = topup_sign(nacq) * displacement;
        displacement = rot90(displacement,1);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
        ...
        if gnl_correction_flag && ~topup_correction_flag
            p1 = double(u + du) / encoded_fov(1) * (2 * pi); % RO [-pi,pi]
            p2 = double(v + dv) / encoded_fov(2) * (2 * pi); % PE [-pi,pi]
        elseif gnl_correction_flag && topup_correction_flag
    ...
    Ah = @(x) type1_pec_sense_adjoint_dual(x, sens_dual, mask_dual, nj, p1_dual, p2_dual, iflag, eps, support_mask_dual, topup_sign, lowres_ph_dual);
    AhA = @(x) type1_pec_sense_normal_dual(x, sens_dual, mask_dual, nj, p1_dual, p2_dual, iflag, eps, support_mask_dual, lambda, topup_sign, lowres_ph_dual);
    ...
    [img, flag, relres, iter, resvec] = pcg(@(x) AhA(x), Ah(ksp_dual), 1e-4, maxiter);
    ...
    % img (Nkx x Nky x Nkz)
    %----------------------------------------------------------------------
    img_filename = sprintf('img_type1_slc%d_rep%d_%s_gridding%d_phc%d_conc%d_gnl%d_topup%d_i%d_l%4.2f', slice_number, repetition_number, slice_type, readout_gridding_flag, phase_correction_flag, conc_correction_flag, gnl_correction_flag, topup_correction_flag, maxiter, lambda);
...
%     %----------------------------------------------------------------------
    ...
    title({'Cartesian Type-1 based SENSE Recon.', sprintf('SLC = %d, REP = %d, %s slice, %s = %4.1f mm', slice_number, repetition_number, slice_type, slice_direction, slice_offset), ...
           sprintf('GNC/topup = %d/%d', gnl_correction_flag, topup_correction_flag), ...
    ...
    drawnow;
    fig_filename = sprintf('img_type1_slc%d_rep%d_%s_gridding%d_phc%d_conc%d_gnl%d_topup%d_i%d_l%4.2f_mag', slice_number, repetition_number, slice_type, readout_gridding_flag, phase_correction_flag, conc_correction_flag, gnl_correction_flag, topup_correction_flag, maxiter, lambda);
    ...
    caxis([-180 180]);
    title({'Cartesian Type-1 based SENSE Recon', sprintf('SLC = %d, REP = %d, %s slice, %s = %4.1f mm', slice_number, repetition_number, slice_type, slice_direction, slice_offset), ...
           sprintf('gridding/PHC/CFC/GNC/topup = %d/%d/%d/%d/%d', readout_gridding_flag, phase_correction_flag, conc_correction_flag, gnl_correction_flag, topup_correction_flag), ...
           sprintf('max. iterations = %d, \\lambda = %4.2f', maxiter, lambda)}, ...
           'FontWeight', 'normal', 'FontSize', FontSize);
    xlabel('x [mm]', 'Interpreter', 'latex', 'FontSize', FontSize);
    ylabel('y [mm]', 'Interpreter', 'latex', 'FontSize', FontSize);
    ...
    set(gca, 'ZDir', 'reverse');
    drawnow;
    fig_filename = sprintf('img_type1_slc%d_rep%d_%s_gridding%d_phc%d_conc%d_gnl%d_topup%d_i%d_l%4.2f_phase', slice_number, repetition_number, slice_type, readout_gridding_flag, phase_correction_flag, conc_correction_flag, gnl_correction_flag, topup_correction_flag, maxiter, lambda);
    export_fig(fullfile(output_path, fig_filename), '-r300', '-tif');
    close gcf;
